...
def occurrence_stitch(*, session: Session, payload: dict) -> None:
    ...
    if existing_link is not None:
        ...
        _enqueue_routing(session=session, org_id=org_id, mailbox_id=UUID(str(occ["mailbox_id"])), occurrence_id=occurrence_id)
        return
    ...
    if oss_ticket_id is not None:
        ...
        _link_message(session=session, org_id=org_id, ticket_id=ticket_id, message_id=message_id, reason="x_oss_ticket_id", confidence=RoutingConfidence.high.value)
        _mark_stitched(session=session, occurrence_id=occurrence_id, ticket_id=ticket_id)
        _enqueue_routing(session=session, org_id=org_id, mailbox_id=UUID(str(occ["mailbox_id"])), occurrence_id=occurrence_id)
        return
    ...
    if ticket_id is not None:
        _link_message(session=session, org_id=org_id, ticket_id=ticket_id, message_id=message_id, reason="reply_to_token", confidence=RoutingConfidence.high.value)
        _mark_stitched(session=session, occurrence_id=occurrence_id, ticket_id=ticket_id)
        _enqueue_routing(session=session, org_id=org_id, mailbox_id=UUID(str(occ["mailbox_id"])), occurrence_id=occurrence_id)
        return
    ...
    if resolved_ticket is not None:
        _link_message(session=session, org_id=org_id, ticket_id=resolved_ticket, message_id=message_id, reason="threading", confidence=RoutingConfidence.medium.value)
    ...
    _link_message(session=session, org_id=org_id, ticket_id=ticket_id, message_id=message_id, reason="new_ticket", confidence=RoutingConfidence.low.value)
    _mark_stitched(session=session, occurrence_id=occurrence_id, ticket_id=ticket_id)
    _enqueue_routing(session=session, org_id=org_id, mailbox_id=UUID(str(occ["mailbox_id"])), occurrence_id=occurrence_id)


def _try_reply_to_token(*, session: Session, org_id: UUID, reply_to_emails: list[str]) -> UUID | None:
...
def _enqueue_routing(*, session: Session, org_id: UUID, mailbox_id: UUID, occurrence_id: UUID) -> None:
    enqueue_job(
        session=session,
